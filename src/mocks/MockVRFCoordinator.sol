...
contract MockVRFCoordinator {
    ...
    function autoFulfillRandomWords(uint256 requestId, uint256 seed) external {
        ...
        randomWords[0] = uint256(keccak256(abi.encode(seed, block.timestamp, 1))) % 100; // Rarity 0-99
        randomWords[1] = uint256(keccak256(abi.encode(seed, block.timestamp, 2))) % 1000; // Lottery 0-999
     ...
     */
    function fulfillWithRarity(uint256 requestId, string memory rarityType) external {
        require(s_consumers[requestId] != address(0), "Invalid request ID");
        require(!s_fulfilled[requestId], "Request already fulfilled");
        ...
        } else if (keccak256(bytes(rarityType)) == keccak256(bytes("legendary"))) {
