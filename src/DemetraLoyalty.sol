...
interface IDemetraShoeNFT {
    ...
    function getTokenMetadata(uint256 tokenId) external view returns (ShoeMetadata.ShoeData memory);
}
...
contract DemetraLoyalty is Ownable {
    ...
    function getUserDiscount(address user) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < userBalance; i++) {
            ...
            ShoeMetadata.ShoeData memory metadata = nftContract.getTokenMetadata(tokenId);
